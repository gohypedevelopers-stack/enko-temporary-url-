...
import { Download, ChevronRight, CheckCircle2, FileText } from 'lucide-react';
        ...
        <div className="container-shell">
          ...
          <div className="mb-16">
            ...
            <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-3 gap-6">
              {[
                { num: 1, name: 'ENKO Flow 7', tag: 'SYSTEM_NODE_01 // AC_SERIES', image: '/products/7.4kw (2).png' },
                { num: 2, name: 'ENKO Flow 11', tag: 'SYSTEM_NODE_02 // AC_SERIES', image: '/products/11kw-22kw.png' },
                { num: 3, name: 'ENKO Flow 22', tag: 'SYSTEM_NODE_03 // AC_SERIES', image: '/products/11kw-22kw.png' },
              ].map((p) => (
                <div key={p.num} className="bg-forge border border-warm/10 rounded-2xl overflow-hidden hover:border-charge/40 transition-all duration-300 group">
                  <div className="relative bg-steel h-52 flex items-center justify-center p-6">
                    <p className="absolute top-3 left-3 text-[7px] font-bold uppercase tracking-[0.2em] text-charge/70">[ {p.tag} ]</p>
                    <Image src={p.image} alt={p.name} fill className="object-contain p-6 group-hover:scale-105 transition-transform duration-500" />
                  </div>
                  <div className="p-5">
                    <h4 className="text-base font-black uppercase text-warm mb-4">{p.num}. {p.name}</h4>
                    <button className="w-full border border-warm/20 hover:border-charge hover:text-charge text-warm/60 text-[10px] font-bold uppercase tracking-widest py-2.5 rounded-lg transition-colors duration-300">
                      [ VIEW TECH SPECS ]
                    </button>
                  </div>
                </div>
          ...
          <div className="mb-16">
            ...
            <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-3 gap-6">
              {[
                { num: 4, name: 'ENKO Storm 30', tag: 'SYSTEM_NODE_04 // DC_FAST_SERIES', image: '/products/30kw.png' },
                { num: 5, name: 'ENKO Storm 60', tag: 'SYSTEM_NODE_05 // DC_FAST_SERIES', image: '/products/60kw.png' },
                { num: 6, name: 'ENKO Storm 120', tag: 'SYSTEM_NODE_06 // DC_FAST_SERIES', image: '/products/120kw.png' },
              ].map((p) => (
                <div key={p.num} className="bg-forge border border-warm/10 rounded-2xl overflow-hidden hover:border-charge/40 transition-all duration-300 group">
                  <div className="relative bg-steel h-52 flex items-center justify-center p-6">
                    <p className="absolute top-3 left-3 text-[7px] font-bold uppercase tracking-[0.2em] text-charge/70">[ {p.tag} ]</p>
          ...
          <div>
            ...
            <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-3 gap-6">
              {[
                { num: 7, name: 'ENKO Blaze 240', tag: 'SYSTEM_NODE_07 // DC_ULTRA_SERIES', image: '/products/240kw.png' },
              ].map((p) => (
                <div key={p.num} className="bg-forge border border-warm/10 rounded-2xl overflow-hidden hover:border-charge/40 transition-all duration-300 group">
                  <div className="relative bg-steel h-52 flex items-center justify-center p-6">
                    <p className="absolute top-3 left-3 text-[7px] font-bold uppercase tracking-[0.2em] text-charge/70">[ {p.tag} ]</p>
